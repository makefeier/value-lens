...
- **v2.1**：LangGraph 降为可选依赖——已安装用 StateGraph 编排，未安装自动降级到内置线性执行器（同一套节点/条件路由/回溯/预算逻辑）。引擎单点分发至独立仓库 doc-pipeline；skill 仓库只保留 sections.json + drafts（规格与草稿是源资产），不再内嵌引擎副本，消除双仓同 md5 的死重。
